...
void GoalChecker::initialize(ros::NodeHandle &nh)
{
  ...
  nh.param(plugin_name_ + "/goal_angular_vel_scaling_angle", goal_angular_vel_scaling_angle_, 0.55);
  ...
  bool &oscillation_state)
{
  ...
  if (check_xy_) {
    if ((hypot_sq(dx, dy) <= xy_goal_tolerance_*xy_goal_tolerance_) || oscillation_state) {
      check_xy_ = false;
    }
    else {
      return false;
  ...
  double angle_to_goal = angles::shortest_angular_distance(
                         curr_robot_pose.pose.theta,
                         transformed_end_pose.pose.theta);
...
bool GoalChecker::hasGoalChanged(
  const nav_2d_msgs::Pose2DStamped &new_goal)
{
  ...
  return last_goal_.pose.x != new_goal.pose.x
         || last_goal_.pose.y != new_goal.pose.y
  ...
  const double & angle_to_path)
{
  ...
  if (std::abs(angle_to_path) < goal_angular_vel_scaling_angle_) {
