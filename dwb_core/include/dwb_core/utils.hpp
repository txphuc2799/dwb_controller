#ifndef DWB_CORE__UTILS_HPP__
#define DWB_CORE__UTILS_HPP__
...
{
  geometry_msgs::PoseStamped in_3d_pose = nav_2d_utils::pose2DToPoseStamped(in_pose);
  geometry_msgs::PoseStamped out_3d_pose;

  try {
    tf->transform(in_3d_pose, out_3d_pose, frame, ros::Duration(transform_tolerance));
  }
  catch(tf2::TransformException& ex)
  {
    ROS_ERROR(
  ...
  out_pose = nav_2d_utils::poseStampedToPose2D(out_3d_pose);
  return true;
}

}  // namespace dwb_core

#endif // DWB_CORE__UTILS_HPP__